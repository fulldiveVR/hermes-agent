...
| `create <name> [--clone] [--clone-all] [--clone-from <source>] [--l7-research] [--no-alias]` | Create a new profile. `--clone` copies config, `.env`, and `SOUL.md` from the active profile. `--clone-all` copies all state. `--clone-from` specifies a source profile. `--l7-research` applies the air-gapped L7 research preset while leaving command approvals unchanged. |
